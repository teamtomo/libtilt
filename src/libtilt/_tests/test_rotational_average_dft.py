...
from libtilt.rotational_averaging.rotational_average_dft import _split_into_frequency_bins_2d, \
...
def test_rotational_average_3d():
    ...
    rotational_average = rotational_average_dft_3d(image)
    ...
    # unshifted rfft
    image = torch.rand(size=(2, 10, 10, 10))
    rotational_average = rotational_average_dft_3d(image, rfft=True, fftshifted=False)
    assert rotational_average.shape == (2, 5)

    # fftshifted rfft
    image = torch.rand(size=(2, 10, 10, 10))
    rotational_average = rotational_average_dft_3d(image, rfft=True, fftshifted=True)
    assert rotational_average.shape == (2, 5)
